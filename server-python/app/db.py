...
-- The three type-ahead queries, each filtered by kind and (for region/city) by
-- the parent chosen in the input above it, then ordered by name.
CREATE INDEX IF NOT EXISTS idx_locations_kind_name    ON locations (kind, name);
CREATE INDEX IF NOT EXISTS idx_locations_kind_country ON locations (kind, country_code, name);
CREATE INDEX IF NOT EXISTS idx_locations_kind_region  ON locations (kind, region_code, name);
